...
## Overview
This skill provides tools to download, process, and visualize NASA VIIRS-SNPP satellite fire detection data for Peru. Tools share state automatically - call them in sequence.
...
**Purpose:** Create interactive choropleth map. Call after intersect_and_collapse_fires_peru.

**Parameters:**
- `out_html` (str, optional): Output file path (default: "peru_viirs_fires.html")
...
5. `folium_choropleth_peru(out_html="fires_2024.html")` - Create map

## Important Notes

- VIIRS-SNPP data available from 2012 to current year
- Tools must be called in sequence (they share state)
- Output maps are interactive HTML files with tooltips
