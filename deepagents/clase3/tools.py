...
def get_session_data():
    """Get the current session data dictionary (for use by the Streamlit app)."""
...
def folium_choropleth_peru() -> str:
    """
    Create an interactive choropleth map showing fire counts by region.
    The map is stored in session for display in the Streamlit chat.
    ...
    -------
    str
        A message confirming the map was created and is ready for display.
    """
    ...
    # Store the map in session data for the Streamlit app to render
    year = _SESSION_DATA.get("fires_year", "unknown")
    _SESSION_DATA["pending_map"] = m
    _SESSION_DATA["map_title"] = f"Peru Wildfire Map ({year})"
    
    return f"Map created showing fire counts for Peru ({year}). The map will be displayed below."
